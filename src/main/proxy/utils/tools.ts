    ...
    for (const msg of messages) {
        if (msg.role === 'system' || msg.role === 'user') {
            ...
            if (hasGeneralToolPromptSignature(content)) {
                console.log(
                    '[Tools] Detected existing tool prompt injection, skipping',
                );
                return true;
            }
        }
...
8. If you are writing code or regular expressions, you MUST properly escape all backslashes and quotes inside the JSON.`;
    }
